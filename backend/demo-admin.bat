@echo off
REM Admin Access Demo Script
REM This script demonstrates the admin endpoint functionality
...
set REGULAR_EMAIL=user@example.com
...
for /f "tokens=*" %%i in ('curl -s -X POST "%BASE_URL%/auth/login" -H "Content-Type: application/json" -d "{\"email\":\"%REGULAR_EMAIL%\",\"password\":\"%PASSWORD%\"}" ^| jq -r ".token"') do set REGULAR_TOKEN=%%i

echo Regular user token: %REGULAR_TOKEN%

echo.
echo Step 4: Login as admin user...
for /f "tokens=*" %%i in ('curl -s -X POST "%BASE_URL%/auth/login" -H "Content-Type: application/json" -d "{\"email\":\"%ADMIN_EMAIL%\",\"password\":\"%PASSWORD%\"}" ^| jq -r ".token"') do set ADMIN_TOKEN=%%i
...
echo Example: set ADMIN_EMAIL=admin@example.com
